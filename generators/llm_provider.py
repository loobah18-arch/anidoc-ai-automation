...
Prioritizes NVIDIA NIM (Nemotron 3 Ultra 550B MoE) -> OpenRouter -> Direct LLMs -> Free Fallback
"""
...
class LLMProvider:
    ...
    def generate(self, prompt: str, system_prompt: str = None, temperature: float = 0.7, max_tokens: int = 4000) -> str:
        ...
        Tier 1: NVIDIA NIM (Nemotron 3 Ultra 550B MoE)
        Tier 2: OpenRouter (DeepSeek / Llama 3.3 / Gemini)
        ...
        if self.nvidia_key:
            try:
                print("🧠 [LLM Engine] Calling Primary Frontier Engine: NVIDIA Nemotron 3 Ultra (550B MoE)...")
                return self._call_nvidia(prompt, system_prompt, temperature, max_tokens)
            except Exception as e:
                print(f"[!] NVIDIA NIM notice ({e}), falling back to Tier 2 (OpenRouter)...")
                errors.append(f"NVIDIA: {e}")

        # Tier 2: OpenRouter
        if self.openrouter_key:
            try:
                print("🧠 [LLM Engine] Calling Tier 2 Engine: OpenRouter...")
                return self._call_openrouter(prompt, system_prompt, temperature, max_tokens)
            except Exception as e:
                print(f"[!] OpenRouter notice ({e}), falling back to Tier 3...")
        ...
        for m in models:
            ...
            try:
                with urllib.request.urlopen(req, timeout=90) as resp:
                    ...
                    print(f"  [✔] Response generated via NVIDIA NIM ({m})")
                    return res_content
            except Exception as e:
                last_err = str(e)
                continue

        raise RuntimeError(f"NVIDIA NIM endpoints failed: {last_err}")
