...
or client_secrets.json to upload full 1080p documentary videos, set high-ranking metadata,
...
class YouTubePublisher:
    ...
    def get_authenticated_service(self):
        ...
        if not all([self.client_id, self.client_secret, self.refresh_token]):
            # Try reading from token file if present
    ...
    def upload_video(
        ...
        # Build metadata export for logging
        meta_export = {
            "title": title[:100],
            "description": description,
            "tags": tags[:30] if isinstance(tags, list) else [t.strip() for t in str(tags).split(",")][:30],
            ...
            "topic_name": topic_name or title
        }

        # Check if credentials are set; if not in dry-run/local mode, export metadata package
        if not all([self.client_id, self.client_secret, self.refresh_token]):
            export_json = video_path.parent / "youtube_upload_metadata.json"
            with open(export_json, "w", encoding="utf-8") as f:
                json.dump(meta_export, f, indent=2, ensure_ascii=False)
            print(f"[!] CLIENT_ID / REFRESH_TOKEN not found in local environment.")
            print(f"    Exported upload metadata package to: {export_json}")
            print(f"    When running on GitHub Actions or with secrets configured, upload will execute automatically.")
            return {
